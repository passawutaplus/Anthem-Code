...
interface Row {
  id: string; slug: string; name: string; tagline: string; verified: boolean;
  member_count: number; location: string; created_at: string;
}

export default function AdminStudiosPage() {
  const { data, isLoading } = useAdminList<Row>("studios");
  const { q, setQ, filtered } = useSearch(data, ["name", "slug", "tagline"]);
  const cols: Column<Row>[] = [
    { key: "name", header: "สตูดิโอ", render: (r) => (
      <div><p className="font-medium">{r.name}</p><p className="text-xs text-admin-muted">/{r.slug}</p></div>
    )},
  ...
  return (
    <div>
      <SectionHeader eyebrow="studios" title="สตูดิโอ" description={`${data?.length ?? 0} สตูดิโอ`} actions={<SearchBar value={q} onChange={setQ} />} />
