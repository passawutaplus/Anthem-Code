...
const ImageLightbox = ({ src, alt, open, onClose }: Props) => {
  ...
  return createPortal(
    <div
      ...
      aria-modal="true"
      onClick={onClose}
      className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md [-webkit-backdrop-filter:blur(12px)] animate-in fade-in duration-200 cursor-zoom-out"
    >
      <button
        type="button"
        onClick={(e) => {
